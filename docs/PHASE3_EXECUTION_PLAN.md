...
- **Objective:** Dual-write the four flat QMS/Risk record tables (deviations, capas, change_controls, risk_assessments) on their frozen-schema fields only (title, status, project_id), plus `audit_trail` entries for the three types that write one today. `attachments`/`comments`/`approvals` dual-write is **deferred** — see below. Behind one shared flag (`QMS_BACKEND`) since all four are one milestone.
- **Files:** `migrations/0008_qms_rls_*.sql` (new), `pharmagpt/db/qms_repo.py` (new, one shared repo for all 4 types), `pharmagpt/config.py` (+`QMS_BACKEND`), `pharmagpt/qms_deviation_database.py` / `qms_capa_database.py` / `qms_change_control_database.py` / `risk_database.py` (+bookkeeping columns/setters), `pharmagpt/routes/qms_deviations.py` / `qms_capa.py` / `qms_change_control.py` / `risk.py`, `scripts/backfill_qms.py` / `check_qms_parity.py` (new).
- **DB impact:** `deviations`/`capas`/`change_controls`/`risk_assessments` populated on title/status/project_id only — `project_id` resolved through `projects.postgres_id` (3.2), falling back to `NULL` if the project wasn't itself dual-written. `risk_assessments` never gets a `project_id` — SQLite has no such column at all (a disclosed roadmap limitation, Phase 7 territory). `audit_trail` populated for deviation/capa/change_control creation only (risk uses its own separate `risk_approval` table, not `qms_audit_trail` — no call site to mirror). **The current build's much richer sub-structures — deviation investigation (fishbone/5-Why/timeline)/impact, CAPA actions/effectiveness, change-control impact/actions/links, risk items/library/actions — have no table in `DATABASE_ARCHITECTURE.md` §4.7's frozen target schema.** Migrating them would mean inventing new Postgres tables, which the "architecture is frozen, do not redesign" instruction rules out — they stay SQLite-only, a documented gap, not a silent one. `attachments`/`comments`/`approvals` dual-write is deferred to a follow-up for the same "don't over-scope a single accelerated pass" reason; `audit_trail` alone already captures every state-changing action with actor/timestamp/reason, the regulator-relevant minimum. *(Amended from the original wording during implementation — see commit history.)*
- **Estimated commits:** 5 (actual: 5). **Rollback point:** Flag off (default) → fully inert.
- **Testing:** 23 new tests (dual-write orchestration across all 4 types + both scripts, fully mocked) + full existing suite, 0 regression. One non-reproducible test-infrastructure flake observed (async extraction thread vs. tmp_path teardown race in an unrelated 3.3 test file), confirmed clean on retry.
- **Success criteria:** Every QMS/Risk workflow (create/update/delete) behaves identically from the UI (verified: no route behavior changed, only additive dual-write side effects gated by a default-off flag); audit trail entries match 1:1 for the three types that produce one.
